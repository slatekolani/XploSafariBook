<div class="tab-pane fade" id="nav-requestSpace" role="tabpanel" aria-labelledby="nav-requestSpace-tab">
    ...
    {{ Form::open(['enctype="multipart/form-data"', 'route' => 'localTourBooking.store', 'autocomplete' => 'off', 'method' => 'post', 'class' => 'needs-validation', 'novalidate']) }}
    @csrf
    <section>
        @php
        $todayDate=Carbon\Carbon::now();
        $tripStartDate=$localTourPackage->safari_start_date;
    @endphp
            ...
            <div class="col-md-12">
                @if ($localTourPackage->TotalSpacesRemainedLabel <= 0)
                    <div class="alert alert-danger rounded-lg shadow-sm" role="alert">
                        ...
                        No space is available in this tour. Please find another ones. <a href="#nav-similarSafaris"
                            class="alert-link">Click here</a> to find similar safaris.
                    </div>
                @elseif($tripStartDate <= $todayDate)
                <div class="alert alert-warning rounded-lg shadow-sm" role="alert">
                    <i class="fas fa-exclamation-circle me-2"></i>
                    Spaces are available! However, the trip post has already expired. Please check out similar safaris <a href="#nav-similarSafaris" class="alert-link">here</a>.
                </div>
                
                @else
                    <div class="alert alert-success rounded-lg shadow-sm" role="alert">
                        <i class="fas fa-check-circle me-2"></i>
                        Space is available! After filling in your details, your budget will automatically get calculated
                        for reference before you request space.
                    </div>
                @endif
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    {{ Form::tel('phone_number', null, ['class' => 'form-control form-control-lg rounded', 'placeholder' => '0...', 'required']) }}
                        ...
                        <div class="section-container mb-4">
                            ...
                            @foreach ($localTourPackageReservations as $key => $localTourPackageReservation)
                                <div
                                    ...
                                    <div class="reservation-images mb-3">
                                        <div class="row g-2">
                                            @foreach (explode(',', $localTourPackageReservation->reservation_images) as $image)
                                                <div class="col-md-3">
                                                    <a href="{{ '/public/' . $image }}"
                                                        data-fancybox="gallery-{{ $key }}" class="d-block">
                                    ...
                                    <div class="d-flex flex-column">
                                        <a href="{{ $localTourPackageReservation->reservation_url }}" target="_blank"
                    ...
                    <div class="section-container mb-4">
                        ...
                        <div class="terms-box p-3 bg-light rounded">
                            ...
                            <ul class="mt-3 text-muted">
                                <li>Discount of <span
                                        style="color: dodgerblue">{{ $localTourPackage->discount_offered }}%</span>
                                    are offered when the total number of travelers you have booked for reaches <span
                                        style="color: dodgerblue">{{ $localTourPackage->number_of_people_for_discount }}
                                        people</span> or more.</li>
                                ...
                                <li> The starting amount required to proceed with the installment
                                    ...
                                    of the discounted amount to be paid before <span
                                        style="color: dodgerblue">{{ date('jS M Y', strtotime($localTourPackage->payment_start_percent_deadline)) }}</span>.
                                ...
                                <li>Cancellation fees may apply according to the cancellation policy if you decide
                                    to cancel your trip. This will cost you <span style="color: dodgerblue"
                                        id="cancellation_fee">...</span>, amounting to <i
                                        style="color:dodgerblue">{{ $localTourPackage->cancellation_percent }}%</i>
                                    of the total cost. After the cancellation deadline of <i
                                        style="color:dodgerblue">{{ date('jS M Y', strtotime($localTourPackage->cancellation_due_date)) }}</i>,
                                    no refund will be issued for cancellations. However, early cancellations after
                                    booking may result in lower cancellation fees than the standard ones. The final
                                    decision is at the discretion of <a
                        ...
                        <div class="terms-box p-3 bg-light rounded">
                            ...
                            <ul class="mt-3 text-muted">
                                ...
                                <li>I have read <i>XploSafariBook</i> <a href="#">Terms & Conditions</a> and
                ...
                <div class="row">
                    ...
                    @auth
                    <input name="user_id" value="{{Auth::user()->uuid}}" hidden>
                    @endauth
                </div>
                <br>
                @if ($localTourPackage->TotalSpacesRemainedLabel <= 0)
                    <div class="alert alert-danger rounded-lg shadow-sm" role="alert">
                        <i class="fas fa-exclamation-circle me-2"></i>
                        No space is available in this tour.
                    </div>
                    @elseif ($tripStartDate <= $todayDate)
                    <div class="alert alert-warning rounded-lg shadow-sm" role="alert">
                        <i class="fas fa-exclamation-circle me-2"></i>
                        Spaces are available for this tour, but the trip post has already expired.
                    </div>
...
@push('after-scripts')
    ...
    <script>
        document.querySelector('.alert-link').addEventListener('click', function(event) {
            event.preventDefault();
            const targetTab = new bootstrap.Tab(document.querySelector('#nav-similarSafaris-tab'));
            targetTab.show();
            document.querySelector('#nav-similarSafaris').scrollIntoView({
                behavior: 'smooth'
            });
        });
    </script>
