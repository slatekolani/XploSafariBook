...
@push('after-scripts')
    ...
    <script  type="text/javascript">
        ...
        $(function() {
            var url = "{{ url("/") }}";
            var table=$('#deleted_tour_packages-table').DataTable({
                processing: true,
                serverSide: true,
                stateSave: true,
                searching: true,
                ...
                initComplete : function () {
                    table.buttons().container().insertBefore('#deleted_tour_packages-table');
                    $("#deleted_tour_packages-table").css("width","100%");
                },
                ...
                ajax: '{{ route('localTourPackages.getDeletedLocalTourPackages',$tourOperator->id) }}',
                columns: [
            ...
            $(document).on('change', '.action_select', function () {
                ...
                if($(this).val() == 2){
                    swal({
                        title: "Warning",
                        text: "Are you sure to delete permanently this tour package?",
                        icon: "warning",
                        buttons: {
                            cancel: "Cancel",
                            confirm: {
