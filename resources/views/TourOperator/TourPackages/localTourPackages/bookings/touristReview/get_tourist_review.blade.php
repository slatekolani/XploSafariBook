<table class="table table-hover table-responsive-md" id="local_tourist_review-table">
    <thead>
    <tr>
...
@push('after-scripts')
    ...
    <script  type="text/javascript">
        ...
        $(function() {
            var url = "{{ url("/") }}";
            var table= $('#local_tourist_review-table').DataTable({
                processing: true,
                serverSide: true,
                stateSave: true,
                searching: true,
                ...
                initComplete : function () {
                    table.buttons().container().insertBefore('#local_tourist_review-table');
                    $("#local_tourist_review-table").css("width","100%");
                },
                ...
                ajax: '{{ route('localTouristReview.getLocalTouristReviews',$localTourBooking->id) }}',
                columns: [
...
&nbsp; &nbsp;
            ...
            $(document).on('click','#approve_or_un_approve_review',function(){
                ...
                $.ajax({
                    type: "GET",
                    dataType: "JSON",
                    url: '{{route('localTouristReview.approveOrUnApproveReview')}}',
                    ...
                    success: function (response) {
                        ...
                        {
                            showNotification(response,'Action not completed. Admin use only');
                        }
                        else{
            ...
            function showNotification(response,message)
            {
                ...
                notify.css({
                    ...
                    'background': response.warning ? '#8B8000' : '#F44336',  // Green for success, red for error
                    'color': '#FFFFFF',
                    'width': '300px',
                    'padding': '15px',
                    'border-radius': '5px',
