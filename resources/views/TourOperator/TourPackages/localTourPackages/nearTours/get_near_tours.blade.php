<table class="table table-hover table-responsive-md" id="near_tours-table" style="background-color: rgba(255,255,255,0.6);color: black">
    <thead>
    <tr>
        <th>@lang('#')</th>
        <th>@lang('Urgency Status')</th>
...
@push('after-scripts')
    ...
    <script  type="text/javascript">
        ...
        $(function() {
            var url = "{{ url("/") }}";
            var table=$('#near_tours-table').DataTable({
                processing: true,
                serverSide: true,
                stateSave: true,
                searching: true,
                ...
                initComplete : function () {
                    table.buttons().container().insertBefore('#near_tours-table');
                    $("#near_tours-table").css("width","100%");
                },
                ...
                ajax: '{{ route('localTourPackages.getNearLocalTours',$tourOperator->id) }}',
                columns: [
                    { data: 'id', name: 'id', orderable: true, searchable: true},
                    { data: 'urgencyStatus', name: 'urgencyStatus', orderable: true, searchable: true},
