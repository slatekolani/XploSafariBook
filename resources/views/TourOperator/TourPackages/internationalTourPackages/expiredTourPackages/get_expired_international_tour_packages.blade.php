<table class="table table-hover table-responsive-md" id="expired_international_tour_packages-table" style="background-color: rgba(255,255,255,0.6);color: black">
    <thead>
    <tr>
        <th>@lang('#')</th>
...
@push('after-scripts')
    ...
    <script  type="text/javascript">
        ...
        $(function() {
            var url = "{{ url("/") }}";
            var table=$('#expired_international_tour_packages-table').DataTable({
                processing: true,
                serverSide: true,
                stateSave: true,
                searching: true,
                ...
                initComplete : function () {
                    table.buttons().container().insertBefore('#expired_international_tour_packages-table');
                    $("#expired_international_tour_packages-table").css("width","100%");
                },
                ...
                ajax: '{{ route('tourPackages.getExpiredInternationalTourPackages',$tourOperator->id) }}',
                columns: [
...
&nbsp; &nbsp;
            ...
            $(document).on('change', '.action_select', function () {
                ...
                if($(this).val() ==3){
                    swal({
                        title: "Warning",
                        text: "Are you sure you want to renew this tour package?",
                        icon: "warning",
                        buttons:{
                            cancel:"cancel",
                            confirm:{
