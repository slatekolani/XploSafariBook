                ...
                @foreach($permissions as $permission)
                    <li>
                        <div class="permissions {{ 'permission_group'. $permission->permission_group_id }}">
                            {{ Form::checkbox('permission'. $permission->id, 'permissions', ($user->permissions()->where('permission_id', $permission->id)->count() > 0) ? 1 : (($permission->getIsUserPermissionRoleAttribute($user->id) == true) ? 1 : 0) , ['class' => 'permission_group'. $permission->permission_group_id, 'id' => 'permission'. $permission->id ,   'aria-describedby' => '', (($permission->getIsUserPermissionRoleAttribute($user->id) == true) ? 'disabled' : '')]) }}
                            {{ Form::label('permission', $permission->display_name   ) }} {!!  $permission->getUserPermissionRoleLabelAttribute($user->id)  !!}
                        </div>
                    </li>
                @endforeach
            </ul>
